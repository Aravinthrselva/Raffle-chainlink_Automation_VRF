...
contract AddConsumer is Script { 
    ...
    function run() external {
            ...
            address raffleContractAddr = DevOpsTools.get_most_recent_deployment("Raffle", block.chainid);

            addConsumerUsingConfig(raffleContractAddr);
    }
     ...
     function addConsumerUsingConfig(address mostRecentlyDeployed) public {
        ...
        addConsumer(subscriptionId, vrfCoordinatorV2, deployerKey, mostRecentlyDeployed);
     }


    function addConsumer(uint64 subId, address vrfCoordinatorV2, uint256 deployerKey, address contractToAddToVrf) public {
        ...
        vm.startBroadcast(deployerKey);
        VRFCoordinatorV2Mock(vrfCoordinatorV2).addConsumer(subId, contractToAddToVrf);
        vm.stopBroadcast();
    }
}
 ...
 uint96 public constant FUND_AMOUNT = 1 ether;
    ...
    function fundSubscription(address vrfCoordinatorV2, uint64 subId, address link, uint256 deployerKey) public {
        ...
        if(block.chainid == 31337) {
            ...
            console.log("Link Token Balance of sender:", LinkToken(link).balanceOf(msg.sender));
